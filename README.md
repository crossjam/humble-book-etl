...
The collection order is stable: descending `end_date_datetime`, then descending bundle ID. The archive view uses keyset pagination over that order. Its first request chooses one offset-aware UTC `snapshot_at` boundary (`Z` or equivalent `+00:00`), and subsequent requests reuse it with the prior page’s `(before_end_date, before_id)` cursor; offset cannot be combined with snapshot/cursor pagination. This is a consistent membership boundary, not an as-of historical reconstruction: records observed or updated after the snapshot are intentionally excluded. The utility fetches all pages with the cursor and deduplicates by bundle ID; interrupted or invalid traversals restart with a new snapshot.
...
Rollout checklist: (1) take and verify a database backup; (2) deploy the schema-compatible backend and let startup apply the additive archive migration; (3) verify schema, row counts, normalized archive states, and migration logs; (4) deploy the frontend; (5) if startup or smoke checks fail, restore the pre-migration backup and previous image. This staged order preserves the compatibility window for older clients.

Implementation checklist: schema migration/backup verification; persistence lifecycle invariants; API response/authentication changes; keyset snapshot pagination and HTTP integration tests; frontend paginated consumption; staged rollout and restore validation.
