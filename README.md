...
The collection order is stable: descending `end_date_datetime`, then descending bundle ID. The archive view uses keyset pagination over that order. Its first request receives one server-issued offset-aware UTC `snapshot_at` boundary in the `X-Snapshot-At` response header (`Z` format), and subsequent requests reuse it with the prior page’s `(before_end_date, before_id)` cursor; offset cannot be combined with snapshot/cursor pagination. This is a consistent membership boundary, not an as-of historical reconstruction: records observed or updated after the snapshot are intentionally excluded. The utility fetches all pages with the cursor and deduplicates by bundle ID. If a page request fails or validation rejects a cursor, the utility shows an error; the user must refresh to restart with a new server-issued snapshot.
